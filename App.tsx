...
const App = () => {
  ...
  const [vertoParams, setVertoParams] = useState({
    webSocket: {
      url: 'wss://ocs.telcovillage.com:8082',
      login: '',
      password: 'Aa11223344',
    },
    deviceParams: {
  ...
  const callbacks = {
    ...
    onCallStateChange: (state: any, callId: string) => {
      console.log(
        '[example] onCallStateChange state.current.name:',
        state.current.name,
        ' - callId:',
        callId,
      );
  ...
  const createVertoInstance = () => {
    const tmpVertoClient = VertoInstanceManager.createInstance(
      ...
      true,
      {
        ...
        autoHangup: true,
        selfManaged: false,
        onNewCallAceppted: onNewCallAccepted,
      },
    );
  ...
  const onNewCallAccepted = (call: Call) => {
    console.log(
      '[App-onNewCallAccepted] answer freeswitch call. call.getId():',
      call.getId(),
    );
    VertoInstanceManager.answerCall(call, {
      callerName: call.getCallerIdentification(),
      from: call.getCallerIdentification(),
      to: call.getCalleeIdentification(),
      useVideo: call.rtc.getHasVideo(),
    });
  };
  ...
  const onLoginHandler = (login: string, password: string, url: string) => {
    ...
    AsyncStorage.setItem('login', JSON.stringify(authParams));
  };

  const onCallRequested = (call: Call) => {
    if (call) {
      console.log(
        '[App-onCallRequested] call.destinationNumber:',
        call.getDestinationNumber(),
        ' - call.calleeIdentification:',
        call.getCalleeIdentification(),
        ' - call.callerIdentification:',
        call.getCallerIdentification(),
      );
    }
  };
  ...
  return (
    <View
      ...
      {
        <View
          style={{maxHeight: 40, marginTop: 20, flex: 1, flexDirection: 'row'}}>
          <Button
            title={vertoClient !== null ? 'Close Socket' : 'Connect'}
            onPress={onChangeSocketState}
          />
          {vertoClient && (
