...
export default class VertinhoClient {
  constructor(
    params = Params,
    vertoCallbacks = defaultVertoCallbacks,
    conferenceCallbacks = {},
  ) {
    this.params = {...params};
  ...
  connectSocket() {
    ...
    this.webSocket.onclose = () => {
      ...
      this.retryingTimer = BackgroundTimer.setTimeout(() => {
        if (this.webSocket != null) this.connectSocket();
      }, 5000);
    };
  ...
  socketReady() {
    if (this.webSocket != null && this.webSocket.readyState === 1) return true;
    else return false;
  }
  ...
  publish(method, params = {}, onSuccess = x => x, onError = x => x) {
    ...
    if (this.webSocket != null) this.webSocket.send(requestStringified);
  }
        ...
        liveArray: new ConferenceLiveArray(
          ...
          event.pvtData.laName,
          {
            onBootstrappedMembers:
              this.conferenceCallbacks.onBootstrappedMembers,
  ...
  handleMessageForCall(data) {
    console.log(
      '🚀 ~ file: VertoClient.js:397 ~ VertinhoClient ~ handleMessageForCall ~ data:',
      data,
    );
    ...
    if (this.calls[data.params.callID]) {
      switch (data.method) {
        case 'verto.bye':
          ...
          this.callbacks.onHangup(data.params.callID);
          break;
        case 'verto.answer':
          existingCall.handleAnswer(data.params.sdp);
          // Uyen
          this.callbacks.onAnswer(data.params);
          break;
        case 'verto.media':
      ...
      const useVideo =
        data.params.sdp && data.params.sdp.indexOf('m=video') > 0;
      const useStereo =
        data.params.sdp && data.params.sdp.indexOf('stereo=1') > 0;
  ...
  destroy() {
    if (this.retryingTimer) clearTimeout(this.retryingTimer);
    if (this.socketReady()) {
    ...
    if (this.webSocket) delete this.webSocket;
    this.webSocket = null;
  }
