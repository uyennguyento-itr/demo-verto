...
const VertoView = (props: Props) => {
  ...
  useEffect(() => {
    ...
    return () => {
      printLog(
        props.showLogs,
        '[vertoView] useEffect unmount props.viewKey:',
        props.viewKey,
      );
  ...
  useEffect(() => {
    printLog(
      props.showLogs,
      '[vertoView] useEffect props.call is null?',
      props.call == null,
    );
    if (props.call) {
  ...
  const initializeVertoClient = () => {
    printLog(
      props.showLogs,
      '[vertoView] initializeVertoClient props.viewKey:',
      props.viewKey,
    );
    setVertoClient(
      VertoInstanceManager.getInstance(props.viewKey, {
        onCallStateChange,
        onNewCall,
        onPlayLocalVideo,
        onPlayRemoteVideo,
  ...
  const getVertoClient = () => {
    printLog(
      props.showLogs,
      '[vertoView] getVertoClient props.viewKey:',
      props.viewKey,
    );
    if (!vertoClient) {
  ...
  const onCallStateChange = (viewKey: string, state: any, callId: string) => {
    ...
    printLog(
      props.showLogs,
      '[vertoView] onCallStateChange viewKey:',
      props.viewKey,
      ' - state:',
      state,
    );
    if (
      state &&
      state.current &&
      (state.current.name === 'hangup' || state.current.name === 'destroy')
    ) {
  ...
  const onNewCall = (viewKey: string, call: Call) => {
    if (!call || !call.getId()) {
      printLog(
        props.showLogs,
        '[vertoView] onNewCall return! call is null?',
        call == null,
      );
      return;
    }
  ...
  const onPlayLocalVideo = (viewKey: string, stream: MediaStream) => {
    ...
    printLog(
      props.showLogs,
      '[vertoView] onPlayLocalVideo viewKey:',
      props.viewKey,
      ' - stream.toURL:',
      stream,
    );
  ...
  const onPlayRemoteVideo = (viewKey: string, stream: MediaStream) => {
    ...
    printLog(
      props.showLogs,
      '[vertoView] onPlayRemoteVideo viewKey:',
      props.viewKey,
      ' - stream.toURL:',
      stream.toURL(),
    );
  ...
  const handleAudioState = () => {
    printLog(
      props.showLogs,
      '[vertoView] handleAudioState props.isAudioOff:',
      props.isAudioOff,
    );
    muteAudio(props.isAudioOff);
  };
  ...
  const handleVideoState = () => {
    printLog(
      props.showLogs,
      '[vertoView] handleVideoState props.isCameraOff:',
      props.isCameraOff,
    );
    muteVideo(props.isCameraOff);
  };
  ...
  const getAudioTrack = (stream: MediaStream): MediaStreamTrack => {
    return (
      stream && stream.getAudioTracks() != null && stream.getAudioTracks()[0]
    );
  };

  const getVideoTrack = (stream: MediaStream): MediaStreamTrack => {
    return (
      stream && stream.getVideoTracks() != null && stream.getVideoTracks()[0]
    );
  };
  ...
  const switchCamera = () => {
    ...
    if (localVideoTrack) {
      printLog(
        props.showLogs,
        '[vertoView - switchCamera] Prepare to switch camera',
      );
  ...
  const callHandler = (callee: string) => {
    ...
    printLog(
      props.showLogs,
      '[vertoView] callHandler newCall is null?',
      newCall == null,
    );
    activeCall.current = newCall;
    printLog(
      props.showLogs,
      '[vertoView] callHandler activeCall is null?',
      activeCall.current == null,
    );
    setCall(newCall);
  ...
  const hangUpHandler = () => {
    printLog(
      props.showLogs,
      '[vertoView] hangUpHandler call is null?',
      call == null,
    );
    if (call && call.getId()) {
  ...
  return (
    <View style={styles.container}>
      {!isStreamStarted ? (
        props.isCallScreenVisible === undefined ||
        props.isCallScreenVisible === false ? (
          props.indicatorVisible === undefined &&
        ...
        <View style={{flex: 1}}>
          ...
          {viewType == ViewType.both && (
            <View style={{flex: 1}}>
              ...
              <ViewContainer
                containerStyle={
                  props.isToolboxVisible
                    ? [
                        styles.localStreamContainer,
                        styles.localStreamContainerUp,
                      ]
                    : styles.localStreamContainer
                }
                objectFit={'contain'}
